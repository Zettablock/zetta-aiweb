...
const debug = createDebug('logger:layouts')
...
type Colour = keyof typeof styles

export function dataFormat(...args: string[]) {
  let index = 1
...
function colorizeStart(style?: Colour) {
  return style ? `\x1B[${styles[style][0]}m` : ''
}

function colorizeEnd(style?: Colour) {
  return style ? `\x1B[${styles[style][1]}m` : ''
}

/**
 * Taken from masylum's fork (https://github.com/masylum/logger-node)
 */
function colorize(str: string, style?: Colour) {
 ...
 */
export function basicLayout(loggingEvent: LoggingEvent) {
  return (
    timestampLevelAndCategory(loggingEvent) + dataFormat(...loggingEvent.data)
  )
}
 ...
 */
export function colouredLayout(loggingEvent: LoggingEvent) {
  return (
    timestampLevelAndCategory(
      loggingEvent,
      loggingEvent.level.colour as Colour
    ) + dataFormat(...loggingEvent.data)
  )
}

export const coloredLayout = colouredLayout

export function messagePassThroughLayout(loggingEvent: LoggingEvent) {
  return dataFormat(...loggingEvent.data)
}

export function dummyLayout(loggingEvent: LoggingEvent) {
 ...
 *  - %o column postion
 *  - %s call stack
 *  - %C class name [#1316](https://github.com/logger-node/logger-node/pull/1316)
 *  - %M method or function name [#1316](https://github.com/logger-node/logger-node/pull/1316)
 *  - %A method or function alias [#1316](https://github.com/logger-node/logger-node/pull/1316)
 *  - %F fully qualified caller name [#1316](https://github.com/logger-node/logger-node/pull/1316)
...
export function patternLayout(
  ...
  tokens = {} as Record<
    string,
    any | ((loggingEvent: LoggingEvent, specifier?: string) => any)
  >
) {
  ...
  function formatMessage(loggingEvent: LoggingEvent, specifier?: string) {
    ...
    if (specifier) {
      ...
      dataSlice = dataSlice.slice(Number(lowerBound), Number(upperBound))
    }
    return dataFormat(...dataSlice)
  }
  ...
  function startColour(loggingEvent: LoggingEvent) {
    return colorizeStart(loggingEvent.level.colour as Colour)
  }

  function endColour(loggingEvent: LoggingEvent) {
    return colorizeEnd(loggingEvent.level.colour as Colour)
  }
  ...
  function pid(loggingEvent?: LoggingEvent) {
    return loggingEvent ? loggingEvent.cluster?.pid || loggingEvent.pid : 0
  }
  ...
  function userDefined(loggingEvent: LoggingEvent, specifier = '') {
  ...
  function contextDefined(loggingEvent: LoggingEvent, specifier = '') {
    ...
    const convertFileURLToPath = function (filepath: string) {
      ...
      if (filepath.startsWith(urlPrefix)) {
        ...
        // backward-compatible for nodejs pre-10.12.0 (without url.fileURLToPath method)
          ...
          if (path.sep === '\\') {
            if (filepath.startsWith(path.sep + path.sep)) {
  ...
  type ReplaceKey = keyof typeof replacers

  function replaceToken(
    conversionCharacter: ReplaceKey,
    loggingEvent: LoggingEvent,
    specifier?: string
  ) {
  ...
  function truncateAndPad(
    toTruncAndPad: string,
    truncation: string,
    padding: string
  ) {
    ...
    while ((result = regex.exec(searchString)) !== null) {
      ...
      if (text) {
        ...
        const replacement = replaceToken(
          conversionCharacter as ReplaceKey,
          loggingEvent,
          specifier
        )
...
export function addLayout(
  name: string,
  serializerGenerator: (config?: any) => any
) {
  layoutMakers[name] = serializerGenerator
}

export function layout(name: string, config?: Configuration) {
